...
public class UserService extends AbstractService<UserMapper, User> implements IService<User> {
    ...
    private void checkBeforeUpdate(UserUpdateRequest userUpdateRequest) {
        ...
        if (StringUtils.isNotBlank(userUpdateRequest.getPhoneNumber())) {
            //更新前手机号去重校验
        ...
        if (StringUtils.isNotBlank(userUpdateRequest.getEmail())) {
            //更新前邮箱去重校验
            if (getIsInUseByEmail(userUpdateRequest.getEmail(), userUpdateRequest.getId())) {
     ...
     * @date 2018/11/24 14:27
     */
    public Boolean getIsInUse(String emailOrPhone) {
    ...
    public Boolean userRegister(UserRegisterRequest userRegisterRequest) {
        //首先校验是否重复
        log.info("校验是否重复");
        checkDuplicate(userRegisterRequest.getUsername(), userRegisterRequest.getEmailOrPhone());
        //校验验证码
        checkValidCode(userRegisterRequest.getEmailOrPhone(), userRegisterRequest.getValidCode());
        //初始化参数
     ...
     */
    public void sendValidCode(String emailOrPhone) throws ClientException, ExecutionException, InterruptedException {
        //判断是邮箱还是手机号的正则表达式
        String em = "^\\w+([-+.]\\w+)*@\\w+([-.]\\w+)*\\.\\w+([-.]\\w+)*$";
        String ph = "^[1][34578]\\d{9}$";
        //生成验证码
        String code = userHelper.gennerateValidCode();
        if (emailOrPhone.matches(em)) {
            //发送邮件
            emailService.sendValidCodeEmail(emailOrPhone, code);
        } else if (emailOrPhone.matches(ph)) {
            //发送手机短信
            smsService.send(emailOrPhone, code);
        } else {
            throw new ServiceException("输入手机号或者邮箱格式不正确");
        }
        userHelper.saveValidCode(emailOrPhone, code);
    }
     ...
     */
    private void checkDuplicate(String username, String emailOrPhone) {
        Integer countName = baseMapper.selectCount(new QueryWrapper<User>()
                .eq("username", username));
        if (countName > 0) {
            throw new ServiceException("该用户名已经被使用，无法添加");
        ...
        if (StringUtils.isNotBlank(emailOrPhone)) {
            Integer countPhone = baseMapper.selectCount(new QueryWrapper<User>()
                    .and(userQueryWrapper -> userQueryWrapper
                            ...
                            .eq("email", emailOrPhone)));
            if (countPhone > 0) {
                throw new ServiceException("该手机号或邮箱已经注册，无法添加");
            }
        }
    }
     ...
     */
    private void checkValidCode(String emailOrPhone, String code) {
        Object validCode = redisUtil.get("SMS_" + emailOrPhone);
        redisUtil.del("SMS_" + emailOrPhone);
        if (Objects.isNull(validCode) || !StringUtils.equals(code, (String) validCode)) {
